...
const users = [
  ...
  { id: "S205", pass: "learn123", name: "Palak Gupta", role: "student" },
];
...
function init() {
  ...
  users
    ...
    .forEach((s) => {
      if (!attendanceStats[s.id])
        attendanceStats[s.id] = { total: 0, present: 0, name: s.name };
    });
  saveData();
  renderLeaderboard();
...
function switchDay(day) {
  ...
  document.getElementById("btn-today").className =
    day === "today" ? "day-btn active" : "day-btn";
  document.getElementById("btn-tomorrow").className =
    day === "tomorrow" ? "day-btn active" : "day-btn";
  renderSlots();
}
...
function renderSlots() {
  ...
  currentSlots.forEach((slot) => {
    ...
    let instructionHtml =
      slot.bookedBy && slot.instructions
        ? `<div class="slot-note"><i class="fa-solid fa-circle-info"></i> ${slot.instructions}</div>`
        : "";
    ...
    if (expired) {
      barColor = "var(--expired)";
      statusText = "Period Over";
    ...
    if (expired) {
      ...
      if (!slot.bookedBy)
        btnHtml = `<button class="btn btn-primary" onclick="window.app.openBookingModal(${slot.id}, 'book')">Book Slot</button>`;
      else if (slot.bookedBy === currentUser.id)
        btnHtml = `<button class="btn" style="background:var(--booked);" onclick="window.app.cancelSlot(${slot.id})">Cancel</button>`;
      else
        btnHtml = `<button class="btn" style="background:var(--text-secondary);" onclick="window.app.openBookingModal(${slot.id}, 'waitlist')">Waitlist</button>`;
    } else {
      if (slot.bookedBy)
        btnHtml = `<div style="color:var(--primary); font-weight:600; margin-top:auto;">Scheduled</div>`;
      else
        btnHtml = `<div style="color:var(--text-secondary); margin-top:auto; font-style:italic;">Free Period</div>`;
    }
...
function openBookingModal(id, action) {
  ...
  if (isSlotExpired(slotsList.find((s) => s.id === id).time))
    return showToast("Time slot has passed.", "error");
...
function confirmBooking() {
  ...
  if (currentActionType === "book") {
    ...
    showToast(
      `Booked for ${viewingDay === "today" ? "Today" : "Tomorrow"}`,
      "success",
    );
  } else {
...
async function cancelSlot(id) {
  const slotsList = scheduleData[viewingDay];
  if (isSlotExpired(slotsList.find((s) => s.id === id).time))
    return showToast("Cannot cancel past classes.", "error");
  ...
  try {
    if (window.api) {
      const slotsForBackend = slotsList.map((s) => ({
        id: s.id,
        bookedBy: s.bookedBy ? parseInt(s.bookedBy.substring(1)) : 0,
        type: s.type,
      }));
      const waitlistForBackend = waitlistData[viewingDay].map((w) => ({
        teacherId: parseInt(w.teacherId.substring(1)),
        slotId: w.slotId,
        type: w.type,
      }));

      const updatedData = await window.api.runScheduler({
        slots: slotsForBackend,
        waitlist: waitlistForBackend,
      });
      ...
      updatedData.forEach((uSlot) => {
        ...
        if (uSlot.bookedBy !== 0 && uSlot.bookedBy !== null) {
          ...
          const wIndex = waitlistData[viewingDay].findIndex(
            (w) => w.teacherId === local.bookedBy && w.slotId === local.id,
          );
          if (wIndex !== -1) {
            local.instructions =
              waitlistData[viewingDay][wIndex].instructions || "";
...
function showToast(message, type = "success") {
  ...
  x.innerHTML = `${icon} <span id="toast-msg">${message}</span>`;
...
function submitAttendance() {
  ...
  checkboxes.forEach((cb) => {
    ...
    if (!attendanceStats[sid])
      attendanceStats[sid] = { total: 0, present: 0, name: "" };
